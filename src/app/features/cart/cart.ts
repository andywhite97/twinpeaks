...
export class Cart implements OnDestroy {
  ...
  constructor(public readonly cart: CartService, private checkout: CheckoutService) {}
  ...
  checkoutWithMomo(): void {
    ...
    this.checkout.startMomoPayment({ ...this.checkoutForm.getRawValue(), items: this.cart.items().map((item) => ({ product_id: item.product.id, quantity: item.quantity })) }).subscribe({
  ...
  private pollPayment(reference: string) {
    ...
    this.statusSubscription = timer(3000, 5000).pipe(
      ...
      next: (payment) => { this.payment.set(payment); if (payment.order_status === 'paid') this.cart.clear(); },
