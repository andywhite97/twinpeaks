...
export class ProductCard {
  ...
  addedToCart = false;
  constructor(private cartService: CartService, private metaTracking: MetaTrackingService) {}
  ...
  toggleCart(event: Event): void { event.stopPropagation(); if (this.product.stock_status === 'out_of_stock') return; if (this.isInCart) { this.cartService.remove(this.product); this.addedToCart = false; return; } if (this.cartService.add(this.product)) { this.metaTracking.trackAddToCart(this.product, 1); this.addedToCart = true; window.setTimeout(() => this.addedToCart = false, 1800); } }
